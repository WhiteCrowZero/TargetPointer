...
from targetpointer.runtime.host_logic import (
    BBox,
    MatchResult,
    apply_deadzone,
    bbox_center,
...
def is_centered_for_shutdown(fields: dict[str, str], center_angle: int, tolerance: int = 2) -> bool:
    current_angle = parse_status_int(fields, "ANGLE")
    target_angle = parse_status_int(fields, "TARGET")
    if current_angle is None:
        return False
    if abs(current_angle - center_angle) > tolerance:
        return False
    if target_angle is None:
        return True
    return abs(target_angle - center_angle) <= tolerance


def safe_shutdown_serial(
    serial_client: PointerSerialClient,
    *,
    center_angle: int,
    response_timeout: float,
    idle_timeout: float,
    settle_timeout: float = 1.8,
    poll_interval: float = 0.08,
    tolerance: int = 2,
) -> None:
    status_fields: dict[str, str] = {}
    try:
        status_fields = parse_status_fields(
    ...
    if attached and not is_centered_for_shutdown(status_fields, center_angle, tolerance):
        try:
            send_control_command(
                serial_client,
                "CENTER",
            ...
            while time.monotonic() < deadline:
                ...
                try:
                    ...
                    break
                if is_centered_for_shutdown(status_fields, center_angle, tolerance):
                    break
...
def should_wait_for_center_completion(
    attached: bool,
    current_output_angle: int | None,
    center_angle: int,
    tolerance: int = 2,
) -> bool:
    if not attached or current_output_angle is None:
        return False
    return abs(current_output_angle - center_angle) > tolerance
...
def main() -> int:
    ...
    try:
        ...
        last_sent_angle, target_angle, _attached = update_angles_from_status_fields(
            status_fields,
        ...
        while True:
            ...
            if center_pending:
                ...
                last_sent_angle, target_angle, attached = update_angles_from_status_fields(
                    status_fields,
                    current_output_angle=last_sent_angle,
                    current_target_angle=target_angle,
                )
                if not attached or is_centered_for_shutdown(status_fields, args.center_angle):
                    center_pending = False
                    state.tracking_state = center_pending_final_state
                    if state.pending_selection is None:
                        target_angle = None
                else:
                    state.tracking_state = STATE_CENTERING

            just_selected = False
            if not center_pending and state.pending_selection is not None:
                    ...
                    if should_stop_for_loss(missed_frames, args.reacquire_frames):
                        ...
                        if loss_command == "CENTER":
                            ...
                            last_sent_angle, target_angle, attached = update_angles_from_status_fields(
                                status_fields,
                                current_output_angle=last_sent_angle,
                                current_target_angle=args.center_angle,
                            )
                            center_pending = should_wait_for_center_completion(
                                attached,
                                last_sent_angle,
                                args.center_angle,
                            )
                            center_pending_final_state = STATE_LOST
                            state.tracking_state = STATE_CENTERING if center_pending else STATE_LOST
                            if not center_pending and state.pending_selection is None:
                                target_angle = None
                        else:
            ...
            if (
                ...
                and smoothed_target_center is not None
                and (just_selected or state.last_match_success)
            ):
            ...
            if tracked_bbox is None and state.tracking_state not in (STATE_LOST, STATE_CENTERING):
            ...
            if key == ord("d"):
                ...
                center_pending_final_state = STATE_SELECTING
                state.tracking_state = STATE_CENTERING if center_pending else STATE_SELECTING
                if not center_pending and state.pending_selection is None:
                    target_angle = None
                if args.verbose and responses:
                    print(responses[-1])
