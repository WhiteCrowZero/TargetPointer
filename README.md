...
- 桌面上位机已具备 Windows 优先的单窗口 UI，可完成摄像头选择、串口连接、目标初始化、状态查看、活动日志查看，以及独立 Insights 分析页查看。
...
桌面上位机当前采用主窗口 + 二级分析页布局：主窗口左侧是实时视频舞台，右侧是连接与控制面板；活动日志默认隐藏，通过 `Activity` 弹层查看；`Insights` 会打开一个独立分析窗口，用于查看目标角、输出角、检测数量和匹配质量的短时趋势。
